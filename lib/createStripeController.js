...
function createStripeController(_options) {
  ...
  function listCards(req, res) {
    ...
    StripeCustomer.findOne({ user_id: user_id }, function (err, customer) {
      ...
      var stripeId = customer.get('stripeId');

      stripe.customers.retrieve(stripeId, function (err, remoteCustomer) {
        if (err) return sendError(res, err, 'Stripe error retrieving payment information');

        stripe.customers.listCards(stripeId, function (err, json) {
          ...
          res.json(_lodash2['default'].map(json.data, function (card) {
            var cardData = _lodash2['default'].pick(card, options.cardWhitelist);
  ...
  function setDefaultCard(req, res) {
    ...
    StripeCustomer.findOne({ user_id: user_id }, function (err, customer) {
      ...
      stripe.customers.update(customer.get('stripeId'), { default_source: cardId }, function (err) {
